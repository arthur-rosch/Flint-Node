...
function countingAdults(passengerList) {
  if (passengerList.includes("adult") == false) {
    console.log(
...
function numberOfStops(passengerList, timeOfTravel) {
  if (Number.isInteger(timeOfTravel) === false || timeOfTravel > 0) {
    ...
    return Math.round((timeOfTravel * 60) / stops);
  } else {
    console.log("O Tempo de viagem deve ser maior que 0 e um numero Float");
  }
}
function expensesPerMeal(passengerList, numberOfStops) {
...
let Array = ["adult"];
console.log(expensesPerMeal(["adult"], numberOfStops(["adult"], 6.5)));
